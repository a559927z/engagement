; (function () {
	...
	Tc.loading = {
		showLoading: function (setting) {
			...
			var _opt = jQuery.extend(true, defSetting, setting);
			var isEmpty = (!$(_opt.region).html()) ? true : false;

			$(_opt.region).append(_opt.template);
			var regionHeight = $(_opt.region).height();
			var regionWidth = $(_opt.region).width();
			var loadTop = isEmpty == true ? 0 : $(_opt.region).height() * -1;


			var offsetX = 60;	//偏移量
			var centerPointX = (regionWidth / 2);	//中心点
			var offsetY = 20;	//偏移量
			var centerPointY = (regionHeight / 2);	// 中心点

			$(_opt.region + ' .load').css({
				'background-color': _opt.bgColor,
				'filter': 'alpha(opacity = ' + _opt.opacity + ')',
				'-moz - opacity': _opt.opacity,
				'-khtml - opacity': _opt.opacity,
				'opacity': _opt.opacity,
				'height': regionHeight,
				'position': 'relative',
				'top': loadTop + 'px'
			});
			$(_opt.region + ' .load .sk-circle').css({
				'position': 'absolute',
				'top': centerPointY - offsetY + 'px',
				'left': centerPointX - offsetX + 'px'
			});
			$(_opt.region + ' .load .txt').css({
				'position': 'absolute',
				'left': centerPointX - offsetY + 'px'
			});
		},
		hideLoading: function (region) {
			$(region + ' .load').remove();
		}
	}
